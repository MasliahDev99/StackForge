...
export class GitOperator implements GitOps {
   ...
   */
  async cloneRepo(urlRepo: string, pathDestino: string, verbose: boolean): Promise<void> {
   ...
   */
  async initRepo(pathDestino: string, verbose: boolean = true): Promise<void> {
    if (!pathDestino) {
   ...
   */
  private async  gitAddAndCommit(pathDestino:string,message:string ='Initial commit', verbose:boolean = true) {
     try{
