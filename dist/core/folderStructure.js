...
function ensureDirectoryExists(dirPath, successMessage) {
    logger_1.logger.info(`Verificando carpeta: ${dirPath}`);
    if (!fs_1.default.existsSync(dirPath)) {
        fs_1.default.mkdirSync(dirPath, { recursive: true });
        logger_1.logger.success(successMessage || `📁 Carpeta creada: ${dirPath}`);
    }
    else {
        logger_1.logger.info(`📁 Ya existe: ${dirPath}`);
    }
}
function createFileIfNotExists(filePath) {
    logger_1.logger.info(`Verificando archivo: ${filePath}`);
    if (!fs_1.default.existsSync(filePath)) {
        fs_1.default.writeFileSync(filePath, '');
        logger_1.logger.success(`📝 Archivo creado: ${filePath}`);
    }
    else {
        logger_1.logger.info(`📝 Ya existe: ${filePath}`);
    }
}
